        ...
        try {
            for(int i = 0 ; i < 10 ; i ++) {
                String animal = FileUtils.readFileToString(TestUtils.loadFixture("fixture/animaldb" +
                        "/animaldb_animal" + i + ".json"), StandardCharsets.UTF_8);
                Response response = client.create(JSONUtils.fromJson(new StringReader(animal)));
                Assert.assertTrue(response.getOk());
            }
            String filter = FileUtils.readFileToString(TestUtils.loadFixture("fixture/animaldb/animaldb_filter.json"), StandardCharsets.UTF_8);
