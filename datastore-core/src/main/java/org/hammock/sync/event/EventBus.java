...
public class EventBus {
    ...
    if (!Modifier.isPublic(listenerClass.getModifiers())) {
        throw new IllegalArgumentException(String.format(Locale.ENGLISH,
            "Event Subscriber class %s needs to be public", listenerClass.getName()));
    }

    List<SubscriberMethod> methods = processClassForSubscriberMethods(listenerClass);
    listeners.put(object, methods);
}

private List<SubscriberMethod> processClassForSubscriberMethods(Class<?> listenerClass) {
    List<SubscriberMethod> methods = new ArrayList<>();
    do {
        processDeclaredMethods(listenerClass, methods);
    } while ((listenerClass = listenerClass.getSuperclass()) != null);

    return methods;
}

private void processDeclaredMethods(Class<?> listenerClass, List<SubscriberMethod> methods) {
    for (Method m : listenerClass.getDeclaredMethods()) {
        if (m.isAnnotationPresent(Subscribe.class)) {
            validateAndAddMethod(listenerClass, m, methods);
        }
    }
}

private void validateAndAddMethod(Class<?> listenerClass, Method m, List<SubscriberMethod> methods) {
    if (!Modifier.isPublic(m.getModifiers())) {
        throw new IllegalArgumentException(String.format(Locale.ENGLISH,
            "Subscriber method %s#%s is required to be public", listenerClass.getName(), m.getName()));
    }

    Class<?>[] params = m.getParameterTypes();
    if (params.length != 1) {
        throw new IllegalArgumentException(String.format(Locale.ENGLISH,
            "Subscriber method %s#%s is required to have only 1 parameter", listenerClass.getName(), m.getName()));
    }

    validateParameterType(listenerClass, m, methods, params[0]);
}

private void validateParameterType(Class<?> listenerClass, Method m, List<SubscriberMethod> methods, Class<?> paramType) {
    if (!Notification.class.isAssignableFrom(paramType)) {
        throw new IllegalArgumentException(String.format(Locale.ENGLISH,
            "Subscriber method %s#%s parameter is required to be assignable %s", listenerClass.getName(),
            m.getName(), Notification.class.getCanonicalName()));
    }

    methods.add(new SubscriberMethod(m, paramType));
}
    }
    ...
    /**
     * Class that encapsulates a {@link Subscribe} annotated method and the
