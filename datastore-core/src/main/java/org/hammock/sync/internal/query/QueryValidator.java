...
import static org.hammock.sync.internal.query.QueryConstants.*;
...
class QueryValidator {
    ...
    if (clause.size() != 1) {
        throw new QueryException(String.format(Locale.ENGLISH, "Clause %s must only have one key", clause));
    }

    String operator = (String) clause.keySet().toArray()[0];
    if (!validOperators.contains(operator)) {
        throw new QueryException(String.format(Locale.ENGLISH, "operator %s is not a valid operator", operator));
    }

    // contains correct operator
    Object clauseOperand = clause.get(operator);
    // delegate the verification according to the operator specifics
    validateOperatorSpecifics(operator, clauseOperand);
}

private static void validateOperatorSpecifics(String operator, Object clauseOperand) throws QueryException {
    switch (operator) {
        case NOT:
            validateNotOperator(clauseOperand);
            break;
        case IN:
            validateInOperator(clauseOperand);
            break;
        default:
...
private static void validateNotOperator(Object clauseOperand) throws QueryException {
    if (!(clauseOperand instanceof Map)) {
        throw new QueryException(String.format(Locale.ENGLISH, "clauseOperand %s is not an instance of Map", clauseOperand));
    }
    validateClause((Map<String, Object>) clauseOperand);
}

private static void validateInOperator(Object clauseOperand) throws QueryException {
    if (!(clauseOperand instanceof List)) {
        throw new QueryException(String.format(Locale.ENGLISH, "clauseOperand %s is not an instance of List", clauseOperand));
    }
...
// Other methods like `validatePredicateValue` and the rest of the class implementation
// would be stated here like the previously submitted code...
