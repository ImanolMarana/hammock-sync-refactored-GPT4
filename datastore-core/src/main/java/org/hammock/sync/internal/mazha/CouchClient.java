...
public class CouchClient {
    ...
    private static SSLCustomizerInterceptor checkAndGetTlsInterceptor() {
        ...
        try {
            if (androidVersionIsLessThan20()) {
                SSLContext defaultSslContext = SSLContext.getDefault();
                if (tlsV1_2IsNotEnabledButSupported(defaultSslContext)) {
                    SSLContext sslContext = SSLContext.getInstance("TLSv1.2");
                    sslContext.init(null, null, null);
                    SSLSocketFactory socketFactory = sslContext.getSocketFactory();
                    return new SSLCustomizerInterceptor(new TlsOnlySslSocketFactory(socketFactory));
                }
            }
        } catch (Exception e) {
    ...
    private static boolean androidVersionIsLessThan20() throws ClassNotFoundException, NoSuchFieldException, IllegalAccessException {
        int androidApiLevel = Class.forName("android.os.Build$VERSION").getField("SDK_INT").getInt(null);
        return androidApiLevel < 20;
    }

    private static boolean tlsV1_2IsNotEnabledButSupported(SSLContext sslContext) {
        ...
        return !Arrays.asList(defaultProtocols).contains("TLSv1.2") && Arrays.asList(supportedProtocols).contains("TLSv1.2");
    }
    ...
    private static final class TlsOnlySslSocketFactory extends SSLSocketFactory {
        ...
        private Socket tlsOnlySocket(Socket socket) {
            if (socket instanceof SSLSocket)
                ((SSLSocket) socket).setEnabledProtocols(new String[]{TLS_V12});
            return socket;
        }
    }

    // Additional methods and classes not shown for brevity
