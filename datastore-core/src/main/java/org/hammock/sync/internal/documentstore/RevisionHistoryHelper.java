...
import org.hammock.sync.internal.android.Base64OutputStreamFactory;
...
public class RevisionHistoryHelper {
    ...
    private static void addAttachments(Map<String, ? extends Attachment> attachments,
                                   ...
                                   boolean shouldInline,
                                   int minRevPos) {
    Map<String, Object> attsMap = prepareAttachmentMap();

    // Maintain the order as it is required for multipart bodies matching
    attachments = new TreeMap<>(attachments);

    for (Map.Entry<String, ? extends Attachment> entry : attachments.entrySet()) {
        String key = entry.getKey();
        SavedAttachment attachment = (SavedAttachment) entry.getValue();
        boolean isNewerThanExisting = attachment.revpos > minRevPos;
        if (!isNewerThanExisting) continue;

        Map<String, Object> attachmentInfoMap = createAttachmentInfo(attachment, shouldInline, minRevPos);
        if (attachmentInfoMap != null) {
            attsMap.put(key, attachmentInfoMap);
        }
    }
...
private static Map<String, Object> prepareAttachmentMap() {
    return new LinkedHashMap<>();
}

private static Map<String, Object> createAttachmentInfo(SavedAttachment attachment, boolean shouldInline, int minRevPos) {
    HashMap<String, Object> attachmentInfo = new HashMap<>();
    attachmentInfo.put("length", attachment.length);
    attachmentInfo.put("encoded_length", attachment.encodedLength);
    attachmentInfo.put("content_type", attachment.type);
    attachmentInfo.put("revpos", attachment.revpos);

    if (attachment.encoding != Attachment.Encoding.Plain) {
        attachmentInfo.put("encoding", attachment.encoding.toString().toLowerCase(Locale.ENGLISH));
    }

    if (shouldHandleAttachmentInline(shouldInline, attachment, minRevPos)) {
        handleInlineAttachment(attachment, attachmentInfo);
    } else {
        attachmentInfo.put("follows", true);
    }

    return attachmentInfo;
}

private static boolean shouldHandleAttachmentInline(boolean shouldInline, SavedAttachment attachment, int minRevPos) {
    return shouldInline && attachment.revpos > minRevPos;
}

private static void handleInlineAttachment(SavedAttachment attachment, Map<String, Object> attachmentInfo) {
    try (InputStream fis = attachment.getInputStream();
        ...
        bos.flush();
        attachmentInfo.put("data", new String(baos.toByteArray(), "UTF-8"));
        attachmentInfo.put("follows", false);
    } catch (IOException e) {
        logger.log(Level.WARNING, "Error processing inline attachment", e);
    }
}
                                   Map<String, Object> outMap,
                                   boolean shouldInline,
