...
public class UnindexedMatcher {
    ...
    if (node instanceof AndQueryNode) {
        return evaluateAndNode((AndQueryNode) node, rev);
    } else if (node instanceof OrQueryNode) {
        return evaluateOrNode((OrQueryNode) node, rev);
    } else if (node instanceof OperatorExpressionNode) {
        return evaluateOperatorNode((OperatorExpressionNode) node, rev);
    } else {
        logUnexpectedNodeError(node);
        return false;
    }
}

private boolean evaluateAndNode(AndQueryNode andNode, DocumentRevision rev) {
    for (QueryNode child : andNode.children) {
        if (!executeSelectorTree(child, rev)) {
            return false;
        }
    }
    return true;
...
private boolean evaluateOrNode(OrQueryNode orNode, DocumentRevision rev) {
...
private boolean evaluateOperatorNode(OperatorExpressionNode node, DocumentRevision rev) {
    Map<String, Object> expression = node.expression;
    String fieldName = expression.keySet().iterator().next();
    Map<String, Object> operatorExpression = (Map<String, Object>) expression.get(fieldName);
    String operator = operatorExpression.keySet().iterator().next();
    ...
    if (invertResult) {
        ...
        operator = operatorExpression.keySet().iterator().next();
    }
    Object expected = operatorExpression.get(operator);
    Object actual = ValueExtractor.extractValueForFieldName(fieldName, rev);
    boolean passed = evaluateExpression(actual, operator, expected);
    return invertResult ? !passed : passed;
}

private void logUnexpectedNodeError(QueryNode node) {
    String msg = String.format("Found unexpected selector execution tree: %s", node);
    logger.log(Level.SEVERE, msg);
}

private boolean evaluateExpression(Object actual, String operator, Object expected) {
    ...
    if (specialCaseOperators.contains(operator)) {
        return valueCompare(actual, operator, expected);
    } else {
        return compareUsingOperator(actual, operator, expected);
    }
}

private boolean compareUsingOperator(Object actual, String operator, Object expected) {
    ...
    for (Object expectedItem : (List<Object>) expected) {
        for (Object actualItem : (List<Object>) actual) {
            if (valueCompare(actualItem, operator, expectedItem)) {
                return true;
            }
        }
    }
