...
import org.hammock.sync.internal.query.ValueExtractor;
...
public class UpdateIndexCallable implements SQLCallable<Void> {
    ...
    List<FieldSort> includeFieldNames = new ArrayList<FieldSort>(initialIncludedFields);
    List<Object> args = new ArrayList<Object>(initialArgs);

    excludeFieldsAlreadyIncluded(fieldNames, initialIncludedFields, includeFieldNames, args, rev);

    ContentValues contentValues = new ContentValues();
    populateContentValues(includeFieldNames, args, contentValues);
...
private void excludeFieldsAlreadyIncluded(List<FieldSort> fieldNames, List<FieldSort> initialIncludedFields, List<FieldSort> includeFieldNames, List<Object> args, DocumentRevision rev) {
    for (FieldSort fieldName : fieldNames) {
...
private void populateContentValues(List<FieldSort> includeFieldNames, List<Object> args, ContentValues contentValues) {
    int argIndex = 0;
    for (FieldSort f : includeFieldNames) {
        String fieldName = String.format("\"%s\"", f.field);
        Object argument = args.get(argIndex++);
        putContentValue(contentValues, fieldName, argument);
    }
}

private void putContentValue(ContentValues contentValues, String fieldName, Object argument) {
    ...
    // Note: No default case - if the type isn't supported, it doesn't get indexed
}
    }
