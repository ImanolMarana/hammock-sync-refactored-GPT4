...
class QueryExecutor {
    ...
    if (node instanceof AndQueryNode) {
        return handleAndQueryNode((AndQueryNode) node, db);
    } else if (node instanceof OrQueryNode) {
        return handleOrQueryNode((OrQueryNode) node, db);
    } else if (node instanceof SqlQueryNode) {
        return handleSqlQueryNode((SqlQueryNode) node, db);
    } else {
        throw new QueryException("Unexpected node type " + node);
    }
}

private Set<String> handleAndQueryNode(AndQueryNode andNode, SQLDatabase db) throws QueryException {
    ...
    for (QueryNode qNode : andNode.children) {
        ...
        if (accumulator == null) {
            accumulator = new HashSet<>(childIds);
        } else {
...
private Set<String> handleOrQueryNode(OrQueryNode orNode, SQLDatabase db) throws QueryException {
    Set<String> accumulator = null;
...
private Set<String> handleSqlQueryNode(SqlQueryNode sqlNode, SQLDatabase db) throws QueryException {
    List<String> docIds = null;
    if (sqlNode.sql != null) {
        docIds = new ArrayList<>();
        SqlParts sqlParts = sqlNode.sql;
        try (Cursor cursor = db.rawQuery(sqlParts.sqlWithPlaceHolders, sqlParts.placeHolderValues)) {
            while (cursor.moveToNext()) {
                String docId = cursor.getString(0);
                docIds.add(docId);
            }
        } catch (SQLException e) {
            String message = "Failed to get a list of document ids";
            logger.log(Level.SEVERE, message, e);
